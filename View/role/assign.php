...
<form id="roleForm" class="form-horizontal" action="?r=role/priv" method="post" onsubmit="return checkForm();">
    <div class="control-group">
        <input type="hidden" name="roleId" value="<?=$roleId?>">
        <input type="hidden" id="newAction" name="newAction"/>
        <select id="srcAction" class="lang_select" multiple="multiple">
            <?php foreach ($srcActions as $id=> $action) { ?>
        ...
        <select id="targetAction" class="lang_select" multiple="multiple">
            <?php foreach ($masterActions as $id=>$action) { ?>
                <option value="<?=$id?>"><?=$action?></option>
            <?php } ?>
        </select>
    </div>
